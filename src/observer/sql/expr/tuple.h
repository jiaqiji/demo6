...
public:
    ...
    void set_speces(std::vector<FieldExpr *> &v) {
        for (FieldExpr *spec : speces_) {
            delete spec;
        }
        speces_.clear();
        for (int k = 0; k < v.size(); ++k) {
            // speces_.push_back(new FieldExpr( v[i]->field()));
        }
    }
...
public:
    ...
    virtual int RowNum() override {
        if (this->tuple_ == nullptr) {
            return 0;
        }
        return this->tuple_->RowNum();
    }
    virtual Tuple *GetRow(int i) override {
        if (this->tuple_ == nullptr) {
            return nullptr;
        }
        RowTuple *pTe = (RowTuple *)this->tuple_->GetRow(i);
        if (pTe) {
            // pTe->set_speces(this->speces_);
        }
        return pTe;
    }
...
class RecordListTuple : public Tuple {
public:
    RecordListTuple() = default;
    virtual ~RecordListTuple() = default;
    virtual int RowNum() override {
        return Records.size();
    }
    void AddRecord(VecRecords &vecs) {
        Records.insert(Records.end(), vecs.begin(), vecs.end());
    }
    ...
    Tuple *GetRow(int i) override {
        if (Records.size() > i) {
            record_ = &Records[i];
            currentRecord.set_record(&Records[i]);
        }
    ...
    void set_schema(const Table *table, const std::vector<FieldMeta> *fields) {
        ...
        currentRecord.set_schema(table,fields);
    }

protected:
    VecRecords Records;
    Record *record_;
    RowTuple currentRecord;
    const Table *table_ = nullptr;
    std::vector<FieldExpr *> speces_;
};
