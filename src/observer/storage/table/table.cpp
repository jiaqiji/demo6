...
RC Table::update_record(Record &record, const char *keyID, const Value *val)
{
  ...
  for (int i = 0; i < table_meta_.field_num(); i++) {
    const FieldMeta *field = table_meta_.field(i + normal_field_start_index);
    if (strcasecmp(keyID, field->name()) == 0) 
    {
      if (field->type() == val->attr_type()) 
      {
          offsetIndex = i;
          break;
      }
  ...
  const FieldMeta *field    = table_meta_.field(offsetIndex + normal_field_start_index);
  size_t           copy_len = field->len();
  if (field->type() == CHARS) {
    const size_t data_len = val->length();
    if (copy_len > data_len) {
      copy_len = data_len + 1;
    }
  }
  memcpy(record_data + field->offset(), val->data(), copy_len);
  return RC::SUCCESS;
}
...
RC Table::delete_record(const Record &record)
{
  ...
  for (Index *index : indexes_) {
    ...
    printf("index->delete_entry\n");
    ASSERT(RC::SUCCESS == rc, 
           "failed to delete entry from index. table name=%s, index name=%s, rid=%s, rc=%s",
           name(), index->index_meta().name(), record.rid().to_string().c_str(), strrc(rc));
  }
  rc = record_handler_->delete_record(&record.rid());
  printf("record_handler_->delete_record\n");
  return rc;
}
...
RC Table::destroy(const char *dir)
{
  ...
  std::string data_file = table_data_file(dir, name());  // std::string(dir) + "/" + name() + TABLE_DATA_SUFFIX;
  chmod(data_file.c_str(), 777);
  // printf("chmod :%s %d\n",data_file.c_str(),res);
  ...
  for (int i = 0; i < index_num; i++) {  // 清理所有的索引相关文件数据与索引元数据
    ...
    printf("index_meta->name():%s", index_meta->name());  // index_meta->name();
